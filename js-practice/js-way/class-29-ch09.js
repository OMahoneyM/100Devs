    ...
    attack(target) {
      if (this.health > 0) {
        ...
        if (target.health > 0) {
          ...
          const bonusXP = 10;
          console.log(
            `${this.name} eliminated ${target.name} and wins ${bonusXP} experience points ${target.gold} gold and ${target.keys} keys(s)`
          );
          this.xp += bonusXP;
...
let accounts = [new Account('Sean'), new Account('Brad'), new Account('George')]

accounts.forEach(e => console.log(e.describe()))
accounts.forEach(e => e.credit(1000))
accounts.forEach(e => console.log(e.describe()))
